...
class Index extends admin_login_check{
    public function index(){
        ...
        //总订单数量
        $o_count = Order::where('id','>=',1)->count();
        //月
        ...
        $yesterday_o_count = Order::where('id','>=',1)
            ...
            ->count();
        //总已支付订单数量
        $pay_count = Order::where('pay_status','<>',0)->count();
        //月
        ...
        $all_profit_order = round($all_profit_order*1000)/1000;
        //平台总收款
        ...
        $month_profit_order = Order::where('pay_status','<>',0)
            ->where('month',$month)
            ->sum('order_price*mt_ratio');
        $month_profit_order = round($month_profit_order*1000)/1000;
        //本月平台收款
        ...
        $day_profit_order = Order::where('pay_status','<>',0)
            ->where('day',$day)
            ->sum('order_price*mt_ratio');
        $day_profit_order = round($day_profit_order*1000)/1000;
        //今日平台收款
        ...
        $yesterday_profit_order = Order::where('pay_status','<>',0)
            ->where('day',$yesterday)
            ->sum('order_price*mt_ratio');
        $yesterday_profit_order = round($yesterday_profit_order*1000)/1000;
        //昨日平台收款
        ...
        //获取商户信息
        $mts = Merchant::field(['mt_name'])->select();
        //交易金额
        $mts_money = [];
        $mts_ratio = [];
        $mts_all_order = [];
